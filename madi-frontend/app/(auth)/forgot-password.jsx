...
export default function ForgotPasswordScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await client.post('/users/forgot-password', { email: email.trim() });
      setSuccess(true);
    } catch (err) {
      setError(err.response?.data?.error ?? '오류가 발생했어요.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <Header title="비밀번호 찾기" onBack={() => router.back()} />
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          {success ? (
            <View style={styles.successBox}>
              ...
              <Button label="로그인으로 돌아가기" onPress={() => router.replace('/(auth)/login')} style={styles.btn} />
            </View>
          ) : (
